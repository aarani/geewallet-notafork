...
type LN() =
    ...
    [<Category "G2G_UpdateFeeAfterSendingMonoHopPayments_Funder">]
    [<Test>]
    member __.``can update fee after sending monohop payments (funder)``() = Async.RunSynchronously <| async {
        ...
        let! updateFeeRes =
            Lightning.Network.UpdateFee (Node.Client clientWallet.NodeClient) channelId newFeeRate
    ...
    [<Category "G2G_UpdateFeeAfterSendingMonoHopPayments_Fundee">]
    [<Test>]
    ...
    member __.``CPFP is used when force-closing channel (funder)``() = Async.RunSynchronously <| async {
        let! channelId, clientWallet, _bitcoind, _electrumServer, _lnd, _fundingAmount =
            try
                OpenChannelWithFundee (Some Config.FundeeNodeEndpoint)
            with
            | ex ->
