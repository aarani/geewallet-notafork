    ...
    let HandleMutualClose
        ...
        async {
            ...
            | Tx (WaitingForFirstConf, closingTx) ->
                let closingDateTime =
                    UnwrapOption (channelStore.TryGetClosingDateTime channelInfo.ChannelId) "BUG: closing time is empty"

                if closingDateTime.Add(TimeBeforeCPFPSuggestion) < DateTime.UtcNow then
                    //FIXME: message
                    Console.WriteLine (sprintf "Channel %s has been mutually-closed but it's taking to loong to get confirmed bla bla do you want to do cpfp." (ChannelId.ToString channelInfo.ChannelId))
