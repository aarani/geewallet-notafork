...
module Account =
    ...
    let CreateAllAccounts (privateKeyBytes: array<byte>) (encryptionPassword: string) (passphrase: array<byte>): Async<unit> = async {
        let encryptedData = KeyStoreUtils.Save privateKeyBytes passphrase encryptionPassword
    ...
    let CheckValidSeed (passphrase: string)
        ...
        async {
            ...
            let encryptedData = KeyStoreUtils.Save masterPrivateKey (Encoding.UTF8.GetBytes passphrase) randomPassword
